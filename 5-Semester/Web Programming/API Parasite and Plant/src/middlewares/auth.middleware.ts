...
async function basicAuthMiddleware(req: Request, res: Response, next: NextFunction) {
   ...
   const user = await repository.findOneBy({"nome": String(authUser)});
   ...
   const isValid = compare(String(authPass),user.senha);
...
function apiKeyAuthMiddleware(req: Request, res: Response, next: NextFunction) {
    ...
    console.log("Header recebido:", apiKeyHeader);
    console.log("Valor esperado (.env):", validApiKey);
